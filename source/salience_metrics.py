...
def random_exclude(range_number, exclude_num):
	list = []
...
def nss(s_map,gt):
# 	gt = discretize_gt(gt)
	s_map = np.interp(s_map, (s_map.min(), s_map.max()), (0,255))
...
def all_metrics(ground_truths, outputs, results, metrics):
	num_cores = int(multiprocessing.cpu_count()/2)
	N = outputs.shape[0]
	
	if 'auc_judd' in metrics:
# 	 	judd_results = []
# 	 	for i in range(N):
# 	  		judd_results.append(auc_judd(outputs[i].squeeze(), ground_truths[i].squeeze()))
		judd_results = Parallel(n_jobs=num_cores)(delayed(auc_judd)(outputs[i].squeeze(), ground_truths[i].squeeze()) for i in range(N))
		results['auc_judd'] += judd_results
	
	
	if 'sauc' in metrics:
	# 	sauc_results = []
	# 	for i in range(N):
	#  		sauc_results.append(auc_shuff(outputs[i].squeeze(), ground_truths[i].squeeze(), ground_truths[random_exclude(N, i)].squeeze()))
		sauc_results = Parallel(n_jobs=num_cores)(delayed(auc_shuff)(outputs[i].squeeze(), ground_truths[i].squeeze(), ground_truths[random.choice([j for j in range(N) if j != i])].squeeze()) for i in range(N))
		results['sauc'] += sauc_results

	if 'cc' in metrics:
	# 	cc_results = []
	# 	for i in range(N):
	#  		cc_results.append(cc(outputs[i].squeeze(), ground_truths[i].squeeze()))
		cc_results = Parallel(n_jobs=num_cores)(delayed(cc)(outputs[i].squeeze(), ground_truths[i].squeeze()) for i in range(N))
		results['cc'] += cc_results
	
	if 'nss' in metrics:
	# 	nss_results = []
	# 	for i in range(N):
	#  		nss_results.append(nss(outputs[i].squeeze(), ground_truths[i].squeeze()))
		nss_results = Parallel(n_jobs=num_cores)(delayed(nss)(outputs[i].squeeze(), ground_truths[i].squeeze()) for i in range(N))
		results['nss'] += nss_results
